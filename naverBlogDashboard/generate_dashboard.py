...
def make_dashboard(config: dict, records: list[dict], today: dt.date) -> str:
    ...
    template = """<!doctype html><html lang='ko'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>네이버 블로그 작성 대시보드</title><style>__CSS__</style></head><body><main><div class='eyebrow'>Naver DataLab · Blog Writer Dashboard</div><h1>오늘의 블로그 글감</h1><p class='sub'>__DATE__ · 대상 독자: __TARGET__</p><div class='meta'><div class='pill'>분석 키워드 <b>__TOTAL__</b>개</div><div class='pill'>데이터 모드 <b>__MODE__</b></div><div class='pill'>추천 글감 <b>__TOP__</b></div></div><section class='details'><h2>절약 관련 제목과 요약</h2><div class='detail-grid'>__DETAILS__</div></section><section class='grid'>__CARDS__</section><div class='table-wrap'><table><thead><tr><th>분야</th><th>키워드</th><th>7일 흐름</th><th>추천도</th><th>판정</th></tr></thead><tbody>__ROWS__</tbody></table></div><p class='notice'>데이터랩 관심도는 상대 지수입니다. 건강·법률·정책 주제는 공식 자료를 확인한 뒤 글을 작성하세요.</p></main><script>const data=__PAYLOAD__;</script></body></html>"""
