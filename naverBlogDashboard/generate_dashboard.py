...
def make_dashboard(config: dict, records: list[dict], today: dt.date) -> str:
    ...
    return f'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>네이버 블로그 작성 대시보드</title><style>
:root{{--bg:#0d1526;--panel:#16223a;--line:#2d4267;--text:#edf4ff;--muted:#9eb0d0;--accent:#78adff;--mint:#68e1ae;--rose:#ff8da8;--amber:#ffd477}}*{{box-sizing:border-box}}body{{margin:0;background:radial-gradient(circle at 80% 0,#29477e,#0d1526 43%);color:var(--text);font:15px/1.5 system-ui,sans-serif}}main{{max-width:1220px;margin:auto;padding:36px 20px 60px}}.eyebrow{{color:var(--accent);font-weight:800;letter-spacing:.1em;font-size:11px;text-transform:uppercase}}h1{{font-size:clamp(28px,5vw,48px);margin:8px 0}}.sub{{color:var(--muted);margin-top:0}}.meta{{display:flex;gap:10px;flex-wrap:wrap;margin:22px 0}}.pill{{background:#ffffff0d;border:1px solid var(--line);border-radius:99px;padding:8px 12px;color:var(--muted)}}.pill b{{color:var(--text)}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}}.card{{background:#16223add;border:1px solid var(--line);border-radius:18px;padding:18px;box-shadow:0 15px 40px #0003}}.card h2{{font-size:17px;margin:0 0 11px}}.rank{{font-size:12px;color:var(--accent)}}.keyword{{font-size:21px;font-weight:850;margin:7px 0}}.category{{color:var(--muted);font-size:12px}}.numbers{{display:flex;gap:18px;margin-top:14px}}.num b{{display:block;font-size:20px}}.num span{{font-size:11px;color:var(--muted)}}.up{{color:var(--mint)}}.down{{color:var(--rose)}}.table-wrap{{overflow:auto;margin-top:20px}}table{{width:100%;border-collapse:collapse;background:#16223a99;border:1px solid var(--line);border-radius:14px;overflow:hidden}}th,td{{padding:12px;text-align:left;border-bottom:1px solid #ffffff10;white-space:nowrap}}th{{color:var(--muted);font-size:12px}}.notice{{margin-top:20px;color:var(--muted);font-size:12px;border-left:3px solid var(--amber);padding-left:12px}}@media(max-width:550px){{main{{padding:25px 14px}}}}
</style></head><body><main><div class="eyebrow">Naver DataLab · Blog Writer Dashboard</div><h1>오늘의 블로그 글감</h1><p class="sub">{{date}} · 대상 독자: {{target}}</p><div class="meta"><div class="pill">분석 키워드 <b>{{total}}</b>개</div><div class="pill">데이터 모드 <b>{{mode}}</b></div><div class="pill">추천 글감 <b>{{top}}</b></div></div><section class="grid">{{cards}}</section><div class="table-wrap"><table><thead><tr><th>분야</th><th>키워드</th><th>7일 흐름</th><th>추천도</th><th>판정</th></tr></thead><tbody>{{rows}}</tbody></table></div><p class="notice">데이터랩 관심도는 상대 지수입니다. 건강·법률·정책 주제는 공식 자료를 확인한 뒤 글을 작성하세요.</p></main><script>const data={payload};document.body.innerHTML=document.body.innerHTML.replaceAll('{{date}}',data.date).replaceAll('{{target}}',data.target).replaceAll('{{total}}',data.records.length).replaceAll('{{mode}}',data.records[0]?.source||'없음').replaceAll('{{top}}',data.records.sort((a,b)=>b.recommendation_score-a.recommendation_score)[0]?.keyword||'없음');</script></body></html>'''.replace("{cards}", "".join(f'<article class="card"><div class="rank">TOP {i+1} · {html.escape(x["category"])}</div><div class="keyword">{html.escape(x["keyword"])}</div><div class="category">{recommendation_text(x)}</div><div class="numbers"><div class="num"><b class="{("up" if x["weekly_change"] >= 0 else "down")}">{x["weekly_change"]:+.1f}%</b><span>7일 흐름</span></div><div class="num"><b>{x["recommendation_score"]:.1f}</b><span>추천도</span></div></div></article>' for i,x in enumerate(sorted(records,key=lambda x:x["recommendation_score"],reverse=True)[:5]))).replace("{rows}", "".join(f'<tr><td>{html.escape(x["category"])}</td><td><b>{html.escape(x["keyword"])}</b></td><td class="{("up" if x["weekly_change"] >= 0 else "down")}">{x["weekly_change"]:+.1f}%</td><td>{x["recommendation_score"]:.1f}</td><td>{recommendation_text(x)}</td></tr>' for x in sorted(records,key=lambda x:(x["category"],-x["recommendation_score"]))))
...
def main():
    ...
    (ROOT / "dashboard.html").write_text(make_dashboard(config, records, today), encoding="utf-8")
